...
export function YuksekGelirlerListesi() {
  const [list, setList] = useState([]);
  const getData = async function () {
    const { data } = await axios.get(
      endPoints.nil_30bin_ustu_gelirler_liste.get
    );
    console.log(data);
    setList(data);
  };
  ...
  const deleteData = async function (id: number | string) {
    const { data } = await axios.post(
      endPoints.nil_otuzbin_uzeri_gelirler_update.post,
      {
        type: 0,
        id,
      }
  ...
  return (
    <Table>
      <TableCaption>30.000 TL Üzeri Gelirler Listesi</TableCaption>
      <TableHeader>
        <TableRow>
